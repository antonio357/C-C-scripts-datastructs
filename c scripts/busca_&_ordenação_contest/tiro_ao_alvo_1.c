...
int its_in(int radius, int x, int y) {
    ...
    a = pow(x,2); b = pow(y,2); c = pow(radius,2);
    if (a + b > c) return False;
    return True;
}

void scored(long long *score, int position_vet_radius) {
...
int binary_search_by_position(const int vet_radius_f[], int position_from_smallest_radius, int x, int y) {
    ...
    int return_position;
    //printf("before get inside the loop\n");
...
int main() {
    int q_circles = 0; int q_shoots = 0; long long total_ps = 0;
    ...
    if (vet_shoots == NULL || vet_radius == NULL) {
      printf("it was not possible to allocated memory\n");
      return - 1;
    }
